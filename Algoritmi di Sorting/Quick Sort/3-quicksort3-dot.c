...
// compilazione: g++ -xc++ lezione4-3-quicksort3-dot.c 
//
// Obiettivo:
...
/// situazione iniziale
      ...
      for (int t=p; t<=r; t++) {
	output_graph << "<TD";
	if (t<k) output_graph << " bgcolor=\"#ffffff\"";  /// non ancora controllati
	if (t==k) output_graph << " bgcolor=\"#ff8080\""; /// pivot
  ...
  while (j <= k){
    ...
    if (graph){
      ...
      for (int t=p; t<=r; t++) {
	output_graph << "<TD";
	if (t<i) output_graph << " bgcolor=\"#e0e0e0\""; /// minori pivot
	if (t>=i && t<j) output_graph << " bgcolor=\"#ff8080\""; /// uguali pivot
	if (t>j && t<=k) output_graph << " bgcolor=\"#ffffff\"";  /// non ancora controllati
	if (t>k) output_graph << " bgcolor=\"#a0a0a0\""; /// maggiori pivot
	output_graph << ">" << A[t] << "</TD>"<<endl;
      }
      output_graph << "</TR>"<< endl;
    }
...
void quick_sort(int* A, int p, int r) {
  ...
  if (p<r) {
	...
	if (p<=p2.x-1) output_graph << "node_before" << p << "_" << p2.x-1 <<"; ";
	output_graph << "node_before" << p2.x << "_" << p2.y <<"; ";
	if (p2.y+1<=r) output_graph << "node_before" << p2.y+1 << "_" << r <<";";
	output_graph << "}"<<endl;
    }
	...
	output_graph << "node_after" << p << "_" << r << "\n";
      ...
      /// per visualizzazione ben organizzata a livelli
      output_graph << "{rank = same; ";	
      if (p<=p2.x-1) output_graph << "node_after" << p << "_" << p2.x-1 <<"; ";
      output_graph << "node_after" << p2.x << "_" << p2.y <<"; ";
      if (p2.y+1<=r) output_graph << "node_after" << p2.y+1 << "_" << r <<";";
      output_graph << "}"<<endl;
