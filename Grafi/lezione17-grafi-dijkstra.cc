...
// compilazione: g++ -xc++ lezione17-grafi-dijkstra.cc; ./a.out -graph; neato graph.dot -Tpdf -o graph.pdf; open graph.pdf
//
// Obiettivo:
...
void shortest_path(int n){
  ...
  while(q_size != 0) {
    ...
    int best_idx = -1;
    for (int i = 0; i < n_nodi; i++) {
      if(V_visitato[i] == 0 &&V_dist[i] < best_dist) { // nodo è in coda ed è migliore del nodo corrente
        best_dist = V_dist[i];
        best_idx = i;
      } 
    }
