...
// 1) eval bottom up di espressione aritmetica in notazione polacca inversa (./a.out 1 2 3 4 + + +)

// 2) analisi elaborazione su stack durante parsing su dot
...
int eval_bottomup(my_stack* s){ 
  ...
  if (op==1){ /// somma
    val1=eval_bottomup(s);
    val2=eval_bottomup(s);
...
int main(int argc, char **argv) {
  ...
  /// il ciclo di parsing legge il prossimo elemento e lo inserisce nello stack

  /// la valutazione dello stack viene eseguita durante l'inserimento
  /// invariante: non esistono >1 operatori nello stack ed e' possibile estrarre (con pop) gli operandi dopo la pop dell'operatore (ricorsione limitata ai due operandi)
  /// il risultato viene re-inserito nello stack
  ...
  while (arg_ct<argc){ /// stringhe non terminate
    ...
    //// la valutazione viene fatta durante le operazioni di inserimento
    //// valutazione dello stack: se c'e' un operatore sono pronto per lavorare sui due operandi successivi!
    int val,op;
    stack_top(s,val,op);
   ...
   /// leggo il valore rimasto nel top dello stack: se la stringa era corretta, top contiene il risultato della valutazione
   int val,op;
   stack_top(s,val,op);  
